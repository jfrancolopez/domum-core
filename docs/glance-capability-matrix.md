...
| Host summary | Hosting | Beszel PocketBase collections | Reviewed custom API or future adapter | Dedicated readonly user exists; usable Glance token unresolved | 5m target | Private-operational | Low | Needs clarification | Pi test proved two approved system IDs and password auth; normal auth token expires in 7 days, Glance v0.8.5 cannot chain login token into a second request, and Beszel universal token did not authorize systems collection access |
