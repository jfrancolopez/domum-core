...
| UniFi counts/health | Network | UniFi API | Reviewed custom API preferred; local adapter only if direct API cannot work safely | Dedicated Glance read-only API key, not Homepage credentials | 5m | Private-operational | Medium | Needs service | Operator approved direct read-only monitoring API key; blocked on task 73 live endpoint/schema review |
