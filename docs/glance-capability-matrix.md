...
| Host summary | Hosting | Beszel PocketBase collections | Reviewed custom API | Dedicated readonly user/token not yet approved | 5m target | Private-operational | Low | Needs clarification | Public source review found authenticated PocketBase collections and readonly role, but no stable OpenAPI/versioned contract; task 70 still needs Pi credential, system-ID, field, stale, and failure testing before task 54 |
