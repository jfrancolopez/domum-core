...
| Host summary | Hosting | Beszel PocketBase collections via local adapter | Future adapter feeding reviewed `custom-api` | Dedicated readonly user exists in `glance-beszel.env`; adapter approved but not implemented | 5m target | Private-operational | Low | Needs clarification | Operator selected a tiny internal adapter path on 2026-08-14. Keep blocked until task 72 proves login, strips disallowed fields, handles unavailable/unauthorized/empty/stale failures, and exposes only summary JSON to Glance |
