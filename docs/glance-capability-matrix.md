...
| Host summary | Hosting | Beszel PocketBase collections via local adapter | Custom API | Dedicated readonly user exists in `glance-beszel.env`; adapter implemented | 5m | Private-operational | Low | Implemented | Beszel summary widget renders bounded status, CPU, memory, load, disk, temperature, freshness, and timestamp when supplied. |
...
| Project releases | Home/Hosting/Technology | GitHub public API | Native `releases` | None/optional token | 6h | Public-safe | Low | Ready | Implemented on Home, Hosting, and Technology; release is not update approval |
...
| GitHub repositories/releases | Home/Technology | GitHub | Native `repository`/`releases` | Optional fine-grained token | 6h | Public-safe | Low | Implemented | Technology shows bounded public Domum Core commits; releases remain on Home/Hosting/Technology. Token only if rate limits require it |
